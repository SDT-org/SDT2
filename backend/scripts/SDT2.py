...
def parse_args():
    ...
    parser.add_argument(
        "--match",
        type=float,
        help="aligner match score"
    )
    parser.add_argument(
        "--mismatch",
        type=int,
        help="aligner mismatch score"
    )
    parser.add_argument(
        "--iog",
        type=int,
        help="internal open gap score"
    )
    parser.add_argument(
        "--ieg",
        type=int,
        help="internal left extend gap score"
    )
    parser.add_argument(
        "--log",
        type=int,
        help="left open gap score"
    )
    parser.add_argument(
        "--leg",
        type=int,
        help="left extend gap score"
    )
    parser.add_argument(
        "--rog",
        type=int,
        help="right open gap score"
    )
    parser.add_argument(
        "--reg",
        type=int,
        help="right extend gap score"
    )
...
def biopython_align(ids, seq_dict, sample_size=3):
    ...
    if any(is_aa(seq) for seq in sampled_seqs):
        aligner = PairwiseAligner()  # Checks for protein sequences
        aligner.match_score = getattr(args, 'match', 1.5)
        aligner.mismatch_score = getattr(args, 'mismatch', -1) or -1
        aligner.target_internal_open_gap_score = getattr(args, 'iog', -2) or -2
        aligner.target_internal_extend_gap_score = getattr(args, 'ieg', -2) or -2
        aligner.target_left_open_gap_score = getattr(args, 'log', -3) or -3
        aligner.target_left_extend_gap_score = getattr(args, 'leg', -2) or -2
        aligner.target_right_open_gap_score = getattr(args, 'rog', -3) or -3
        aligner.target_right_extend_gap_score = getattr(args, 'reg', -2) or -2
        aligner.query_internal_open_gap_score = getattr(args, 'iog', -2) or -2
        aligner.query_internal_extend_gap_score = getattr(args, 'ieg', -2) or -2
        aligner.query_left_open_gap_score = getattr(args, 'log', -3) or -3
        aligner.query_left_extend_gap_score = getattr(args, 'leg', -2) or -2
        aligner.query_right_open_gap_score = getattr(args, 'rog', -3) or -3
        aligner.query_right_extend_gap_score = getattr(args, 'reg', -2) or -2
    else:
        aligner = PairwiseAligner()
        aligner.match_score = getattr(args, 'match', 1.5)
        aligner.mismatch_score = getattr(args, 'mismatch', -1) or -1
        aligner.target_internal_open_gap_score = getattr(args, 'iog', -3) or -3
        aligner.target_internal_extend_gap_score = getattr(args, 'ieg', -2) or -2
        aligner.target_left_open_gap_score = getattr(args, 'log', -4) or -4
        aligner.target_left_extend_gap_score = getattr(args, 'leg', -3) or -3
        aligner.target_right_open_gap_score = getattr(args, 'rog', -4) or -4
        aligner.target_right_extend_gap_score = getattr(args, 'reg', -3) or -3
        aligner.query_internal_open_gap_score = getattr(args, 'iog', -3) or -3
        aligner.query_internal_extend_gap_score = getattr(args, 'ieg', -2) or -2
        aligner.query_left_open_gap_score = getattr(args, 'log', -4) or -4
        aligner.query_left_extend_gap_score = getattr(args, 'leg', -3) or -3
        aligner.query_right_open_gap_score = getattr(args, 'rog', -4) or -4
        aligner.query_right_extend_gap_score = getattr(args, 'reg', -3) or -3
...
def get_alignment_scores(seq_dict, args):
    ...
    with Pool(processes=num_processes) as pool:
        results = pool.map(bound_align_and_score, combos)
...
def output_summary(aligner_params, aligner_alg, file_name,start_run , end_run, run_summary, INPUT_PATH):
